...
class MacroUI:
    def __init__(self, root: tk.Tk):
        ...
        self.macro_listbox = tk.Listbox(left_frame)
    ...
    def load_file(self):
        messagebox.showinfo("불러오기", "파일 불러오기 기능은 아직 구현되지 않았습니다.")

    def save_file(self):
        messagebox.showinfo("저장하기", "파일 저장 기능은 아직 구현되지 않았습니다.")
    ...
    def open_settings(self):
        ...
        # 실행/중지 버튼 (설정창 내)
        btns = tk.Frame(frm)
        btns.grid(row=row+1, column=0, columnspan=3, pady=12)
        tk.Button(btns, text="▶ 실행", width=10, command=self.apply_and_run).grid(row=0, column=0, padx=6)
        tk.Button(btns, text="■ 중지", width=10, command=self.stop_execution).grid(row=0, column=1, padx=6)

        # 닫기
        tk.Button(frm, text="닫기", command=lambda: self._close_settings(win)).grid(row=row+2, column=0, columnspan=3, pady=6)

        # 키 바인딩
        win.bind("<Return>", lambda e: self.apply_and_run())
    ...
    def add_keyboard(self):
        ...
        def on_key(event):
            ...
            self.macro_listbox.insert(tk.END, f"키보드:{key}:{action}")
            key_window.destroy()
    ...
    def add_mouse(self):
        ...
        def capture():
            ...
            info.config(text=f"캡처됨: ({x}, {y})  /  버튼: {btn_var.get()}")
        ...
        def add_item():
            if captured["x"] is None:
                ...
                return
            b = btn_var.get()
            self.macro_listbox.insert(tk.END, f"마우스:{captured['x']},{captured['y']}:{b}")
            on_close()
    ...
    def add_delay(self):
        ...
        if sec:
            self.macro_listbox.insert(tk.END, f"시간:{sec}")
    ...
    def delete_macro(self):
        ...
        if selected:
            self.macro_listbox.delete(selected)
        else:
    ...
    def _finish_execution(self):
        ...
        messagebox.showinfo("완료", "매크로 실행이 종료되었습니다.")
